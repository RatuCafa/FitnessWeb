...
// Fungsi pembantu untuk membuat format string tanggal YYYY-MM-DD lokal
const formatDateString = (year, monthIndex, dayStr) => {
  const pad = (num) => String(num).padStart(2, '0');
  return `${year}-${pad(monthIndex + 1)}-${pad(parseInt(dayStr, 10))}`;
};
...
const generateCurrentWeek = () => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    daysArr.push({
      ...
      fullDateStr: formatDateString(d.getFullYear(), d.getMonth(), d.getDate().toString())
    });
  }
  return daysArr;
};
...
const Jadwal = () => {
  ...
  // State selectedDate memegang format penuh YYYY-MM-DD untuk dicocokkan ke database MySQL
  const [selectedDate, setSelectedDate] = useState(
    formatDateString(today.getFullYear(), today.getMonth(), today.getDate().toString())
  );
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [scheduledWorkouts, setScheduledWorkouts] = useState([]); // Array penampung jadwal dari DB
  const [modalPrograms, setModalPrograms] = useState([]); // Array penampung list program untuk modal popup
  ...
  const fetchJadwalByTanggal = (tanggal) => {
    fetch(`http://localhost:5000/api/jadwal/${tanggal}`)
      ...
      .then(data => {
        // PENGAMAN JALUR DATA INFOMATIKA: Pastikan respons beneran array sebelum dimasukkan ke state
      ...
      .catch(err => {
        ...
        setScheduledWorkouts([]); // Set ke array kosong jika backend error agar web tidak putih
      });
  };
  ...
  // 2. Ambil data program workout asli dan total progress lokal saat halaman pertama kali dimuat
  useEffect(() => {
    ...
    const localCount = parseInt(localStorage.getItem('completedWorkouts') || '0', 10);
    setTotalSelesai(localCount);
  }, []);

  // 3. Fungsi mengubah status selesaian latihan (PUT request)
  const handleToggleComplete = (id, currentStatus) => {
    ...
    .then(() => {
      setScheduledWorkouts(prev => prev.map(w => 
        w.id === id ? { ...w, is_completed: nextStatus } : w
      ));
      setTotalSelesai(prev => nextStatus === 1 ? prev + 1 : Math.max(0, prev - 1));
    })
    .catch(err => console.error("Gagal mengubah status tugas:", err));
  };

  // 4. Fungsi menghapus jadwal dari database (DELETE request)
  ...
  // 5. Fungsi menambahkan jadwal baru lewat modal popup (POST request)
 // Di file Jadwal.jsx kamu, cari fungsi handleAddWorkout
// Cara 2: Di file Jadwal.jsx, ganti isi handleAddWorkout menjadi seperti ini
const handleAddWorkout = (programId) => {
  ...
  .then(res => res.json())
  .then((response) => {
    // SINKRONISASI DATANYA SECARA MANUAL (OPTIMISTIC UPDATE):
    // Cari dulu data program asli dari database (misal 'Fat Loss') berdasarkan programId
    const programAsli = modalPrograms.find(prog => prog.id === programId);
    
    if (programAsli) {
      // Buat objek jadwal baru yang formatnya sama persis dengan kiriman MySQL JOIN
      const jadwalBaru = {
        id: response.id, // Gunakan ID baru dari response backend
        is_completed: 0,
      ...
      // Update state scheduledWorkouts dengan data baru ini secara instan
    ...
    <div className="bg-transparent text-white relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          ...
          {/* SINKRONISASI VALIDASI ARRAY SUPAYA ANTI-BLANK SCREEN */}
          {!Array.isArray(scheduledWorkouts) || scheduledWorkouts.length === 0 ? (
            ...
            scheduledWorkouts.map(workout => (
              <div
                key={workout.id}
                className={`bg-[#1A2E35] p-4 md:p-6 rounded-2xl flex justify-between items-center mb-3 shadow-lg hover:translate-x-1 transition-all ${workout.is_completed === 1 ? 'border border-green-500/60 shadow-green-900/20' : ''}`}
              >
                <div className="flex items-center gap-3 md:gap-5">
                  {workout.is_completed === 1 ? (
                    <CheckCircle2
                      onClick={() => handleToggleComplete(workout.id, workout.is_completed)}
                  ...
                  <div className="flex flex-col">
                    <span className={`text-base md:text-xl mb-1 font-bold ${workout.is_completed === 1 ? 'text-zinc-400 line-through' : 'text-white'}`}>
                      {workout.nama_program}
                    </span>
                    <span className={`text-sm font-medium tracking-wide ${workout.is_completed === 1 ? 'text-zinc-600' : 'text-zinc-400'}`}>
          ...
          <div className="bg-[#1A2E35] p-5 md:p-8 rounded-[1.5rem] md:rounded-[2rem] shadow-xl text-white h-full">
            ...
            <div className="mb-8">
              ...
              <div className="flex items-baseline">
                <span className="text-4xl md:text-6xl font-black tracking-tighter">{totalSelesai}</span>
      ...
      {/* Modal Overlay Tambah Jadwal Latihan */}
      {isModalOpen && (
  ...
  {modalPrograms?.map((prog) => (
    <div
      key={prog.id}
      // PASTIKAN BARIS INI ADA DI DIV UTAMA KARTU:
