...
const Dashboard = () => {
  const [stats, setStats] = useState({ totalTime: '1:30', totalSessions: 12, targetSessions: 8 });

  // TODO (Backend): Fetch user stats API here and update setStats.
  useEffect(() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 min-h-[450px]">
        ...
        <div className="bg-[#1A2E35] p-5 md:p-8 rounded-3xl flex flex-col justify-between h-full min-h-[300px]">
          ...
          <div className="relative flex justify-center items-center flex-1 my-4">
            <svg className="w-40 h-40 md:w-48 md:h-48 -rotate-90 origin-center drop-shadow-lg" viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="40" fill="transparent" strokeWidth="12" className="stroke-red-500" strokeDasharray="251.2" strokeDashoffset="75.36" strokeLinecap="round" />
            </svg>
            <div className="absolute flex flex-col items-center justify-center">
              <span className="text-4xl md:text-5xl font-bold text-white">{stats.totalTime}</span>
              <span className="text-xs md:text-sm text-zinc-400 font-bold uppercase tracking-wider mt-1">HOURS</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#1A2E35] p-5 md:p-8 rounded-3xl flex flex-col justify-between h-full min-h-[300px]">
          ...
          <div className="relative flex justify-center items-center flex-1 my-4">
            <svg className="w-40 h-40 md:w-48 md:h-48 -rotate-90 origin-center drop-shadow-lg" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="40" fill="transparent" strokeWidth="12" className="stroke-gray-700" />
              <circle cx="50" cy="50" r="40" fill="transparent" strokeWidth="12" className="stroke-emerald-500" strokeDasharray="251.2" strokeDashoffset="50.24" strokeLinecap="round" />
            </svg>
            <div className="absolute flex flex-col items-center justify-center">
              <span className="text-4xl md:text-5xl font-bold text-white">{stats.totalSessions}<span className="text-xl md:text-2xl text-zinc-400">/{stats.targetSessions}</span></span>
